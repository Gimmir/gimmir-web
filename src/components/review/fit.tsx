...
export function FitSection({
  ...
  const notFits = data.fitNotItems ?? [];

  return (
    <section className="border-t border-line py-20 md:py-28">
      <Container>
        <Reveal>
          <SectionHeader index="07" title={data.fitHeading} />
        </Reveal>

        <div
          className={`mt-12 grid gap-6 md:items-stretch ${notFits.length ? "md:grid-cols-2" : ""}`}
        >
          {/* a fit */}
          ...
          {/* not a fit (hidden until the list has content) */}
          {notFits.length ? (
